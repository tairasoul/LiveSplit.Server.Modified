                ...
                switch (command)
                {
                    ...
                    case "setcurrentsplitname":
                        ...
                        if (command == "setsplitname")
                        {
                            ...
                            //splitTime = new Time(TimeSpan.Parse(options[2]));
                            //pbTime = new Time(TimeSpan.Parse(options[3]));
                        }
                        ...
                        if (index >= 0 && index < State.Run.Count)
                        {
                            ...
                            /*if (splitTime != null)
                            {
                                State.Run[index].SplitTime = splitTime.Value;
                            }
                            if (pbTime != null)
                            {
                                State.Run[index].PersonalBestSplitTime = pbTime.Value;
                            }*/
                        }
                        else if (index >= 0)
                        {
                            State.Run.AddSegment(title);
                            State.Run.HasChanged = true;
                        }
                        break;
                    case "clearsplits":
